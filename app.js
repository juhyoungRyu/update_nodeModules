"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
// import fs module
var promises_1 = require("fs/promises");
var fs_1 = require("fs");
var shell = require("shelljs");
// import path module
var path = require("path");
// Constant Value
var DIRECTORY_PATH = "C:/Users/jhryu/Documents/cals/LambdaV2";
/**
 * @desc
 * 삭제 대상 이름 상수
 */
var TARGET_NAME_ARRAY = ["node_modules", "pnpm-lock.yaml"];
// logger util
function logger(message, type, optionMessage) {
    switch (type) {
        case "start":
            if (message.includes("Update")) {
                console.log("");
            }
            console.log("\uD83D\uDCE2 Start : [".concat(message, "]"));
            if (message === "Process") {
                console.log("\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161");
            }
            break;
        case "end":
            if (message === "Process") {
                console.log("\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161\u3161");
            }
            console.log("\uD83D\uDCE2 End : [".concat(message, "]").concat(optionMessage ? " | ".concat(optionMessage) : ""));
            if (message.includes("Update")) {
                console.log("");
            }
            break;
        case "success":
            console.log("\u2705 Success [".concat(message, "]").concat(optionMessage ? " | ".concat(optionMessage) : ""));
            break;
        case "info":
            console.log("\u2611\uFE0F  Info : [".concat(message).concat(optionMessage ? " - " + optionMessage : "", "]"));
            break;
        case "install":
            if (message.includes("Start")) {
                console.log("");
            }
            console.log("\u2B50 Install : ".concat(message));
            if (message.includes("End")) {
                console.log("");
            }
            break;
        case "error":
            console.log("\u26A0\uFE0F  Error : [".concat(message, "]"));
            break;
    }
}
/**
 * makeDirList
 * @desc 지정된 Path에 존재하는 폴더들의 이름을 배열화하여 리턴
 * @returns string[]
 */
function makeDirList() {
    return __awaiter(this, void 0, void 0, function () {
        var files, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, (0, promises_1.readdir)(DIRECTORY_PATH)];
                case 1:
                    files = _a.sent();
                    files = files.filter(function (file) { return file !== "updateAllLambda"; });
                    logger("Directory reading", "success");
                    return [2 /*return*/, files];
                case 2:
                    err_1 = _a.sent();
                    logger("Directory reading", "error");
                    return [2 /*return*/, []];
                case 3: return [2 /*return*/];
            }
        });
    });
}
/**
 * removeTarget
 * @desc 대상 파일 혹은 폴더를 삭제
 * @param folderName 삭제를 수행할 폴더 경로
 * @returns Promise<boolean>
 */
function removeTarget(folderName) {
    return __awaiter(this, void 0, void 0, function () {
        var i, TARGET_PATH;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    // package.json이 없으면 진행하지 않음
                    if (!(0, fs_1.existsSync)(path.join(DIRECTORY_PATH, folderName, "package.json"))) {
                        logger("package.json Not Found", "error");
                        logger("Update : ".concat(folderName), "end");
                        return [2 /*return*/, false];
                    }
                    i = 0;
                    _a.label = 1;
                case 1:
                    if (!(i < TARGET_NAME_ARRAY.length)) return [3 /*break*/, 5];
                    TARGET_PATH = path.join(DIRECTORY_PATH, folderName, TARGET_NAME_ARRAY[i]);
                    if (!(0, fs_1.existsSync)(TARGET_PATH)) return [3 /*break*/, 3];
                    logger("".concat(TARGET_NAME_ARRAY[i], " :  Exist"), "info");
                    return [4 /*yield*/, (0, promises_1.rm)(TARGET_PATH, { recursive: true })];
                case 2:
                    _a.sent();
                    logger("delete : ".concat(TARGET_NAME_ARRAY[i]), "success");
                    return [3 /*break*/, 4];
                case 3:
                    logger("".concat(TARGET_NAME_ARRAY[i], " : Not Exists"), "info");
                    _a.label = 4;
                case 4:
                    i++;
                    return [3 /*break*/, 1];
                case 5:
                    logger("remove", "success");
                    return [2 /*return*/, true];
            }
        });
    });
}
/**
 * installNodeModules
 * @desc 새로운 node_modules 설치
 * @param folderName 설치를 수행할 폴더명
 * @returns Promise<boolean>
 */
function installNodeModules(folderName) {
    return __awaiter(this, void 0, void 0, function () {
        var cwd;
        return __generator(this, function (_a) {
            cwd = path.join(DIRECTORY_PATH, folderName);
            logger("working in", "info", cwd);
            logger("[node_modules] Start", "install");
            try {
                shell.exec("cd ".concat(cwd, " && pnpm i"));
                logger("[node_modules] End", "install");
                return [2 /*return*/, true];
            }
            catch (error) {
                logger("".concat(error.message), "error");
                return [2 /*return*/, false];
            }
            return [2 /*return*/];
        });
    });
}
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var lambdaDirList, i, step1, step2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, makeDirList()];
                case 1:
                    lambdaDirList = _a.sent();
                    // Error Case #1
                    if (lambdaDirList.length === 0) {
                        return [2 /*return*/];
                    }
                    logger("Process", "start");
                    i = 0;
                    _a.label = 2;
                case 2:
                    if (!(i < lambdaDirList.length)) return [3 /*break*/, 6];
                    logger("Update : ".concat(lambdaDirList[i]), "start");
                    return [4 /*yield*/, removeTarget(lambdaDirList[i])];
                case 3:
                    step1 = _a.sent();
                    if (!step1) {
                        return [3 /*break*/, 5];
                    }
                    return [4 /*yield*/, installNodeModules(lambdaDirList[i])];
                case 4:
                    step2 = _a.sent();
                    if (!step2) {
                        return [3 /*break*/, 5];
                    }
                    logger("Update : ".concat(lambdaDirList[i]), "end");
                    _a.label = 5;
                case 5:
                    i++;
                    return [3 /*break*/, 2];
                case 6:
                    logger("Process", "end");
                    return [2 /*return*/];
            }
        });
    });
}
main();
